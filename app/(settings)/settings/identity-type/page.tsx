...
import { useGetIdentityTypesQuery } from "lib/redux/feesApi"
import { notify } from "components/ui/Notification/Notification"
import { MdOutlineArrowBackIosNew, MdOutlineArrowForwardIos } from "react-icons/md"

interface DisplayIdentityType {
  id: number
...
export default function IdentityTypes() {
  ...
  const { data, error, isLoading } = useGetIdentityTypesQuery()

  const identityTypes: DisplayIdentityType[] = data?.data
    ? data.data.map((type, i) => ({
        id: i + 1,
        identityTypeID: type.identityTypeID,
        identityTypeName: type.identityTypeName,
        identityTypeObject: type.identityTypeObject,
      }))
    : []
  ...
  const currentItems = identityTypes.slice(indexOfFirstItem, indexOfLastItem)
  const totalPages = Math.ceil(identityTypes.length / itemsPerPage)
  ...
  const handleAddIdentityType = async (formData: { name: string; code: string }) => {
    setIsSubmitting(true)
    try {
      // TODO: Implement API call to add identity type
      await new Promise((resolve) => setTimeout(resolve, 1000))
      notify("success", "Identity type added successfully")
      setIsAddModalOpen(false)
    } catch (error) {
      notify("error", "Failed to add identity type")
      console.error(error)
    } finally {
  ...
  const handleSuccess = () => {
    // This will trigger a refresh of the identity types list
    setIsAddModalOpen(false)
  }
    ...
    return (
      <section className="h-full w-full">
        ...
        <div className="flex h-screen items-center justify-center">
          <div className="text-red-500">Failed to load identity types</div>
        </div>
      </section>
    )
  }
      ...
      <div className="flex min-h-screen w-full flex-col">
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between border-b px-16 py-4 max-sm:px-3">
            <p className="text-2xl font-medium">Identity Types</p>
            ...
            <div className="w-full">
              <div className="mb-5 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm max-sm:text-xs">
                  <p className="text-grey-300">Identity Types:</p>
                  {isLoading ? (
                    <div className="h-4 w-8 animate-pulse rounded bg-gray-200"></div>
                  ) : (
                    <p>{identityTypes.length}</p>
                  )}
                </div>
              </div>
                      ...
                      <div key={type.id} className="rounded-lg bg-white p-4 shadow-lg">
                        <div className="flex gap-4">
                          <div className="bg-primary/10 relative flex h-12 w-12 items-center justify-center rounded-full">
                            <span className="text-primary text-lg font-semibold">{type.identityTypeObject}</span>
                          </div>
                          <div className="flex-1">
                            <h3 className="text-base font-bold">{type.identityTypeName}</h3>
                            <p className="text-sm text-gray-500">ID: {type.identityTypeID}</p>
                            <p className="text-sm text-gray-500">Code: {type.identityTypeObject}</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {/* Pagination */}
              {!isLoading && identityTypes.length > itemsPerPage && (
                <div className="flex items-center justify-between border-t px-4 py-3">
                  <div className="text-sm text-gray-700">
                    Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, identityTypes.length)} of{" "}
                    {identityTypes.length} entries
                  </div>
