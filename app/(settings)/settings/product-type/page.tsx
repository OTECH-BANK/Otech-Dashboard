...
import { useGetProductTypesQuery } from "lib/redux/api"

interface DisplayProductType {
  id: number
...
export default function ProductTypes() {
  ...
  const { data, error, isLoading } = useGetProductTypesQuery()

  const productTypes: DisplayProductType[] = data?.data
  ...
  const handleAddProductType = async (formData: { name: string; code: string; accountType: number }) => {
    setIsSubmitting(true)
    try {
      // TODO: Implement API call to add product type
      await new Promise((resolve) => setTimeout(resolve, 1000))
      notify("success", "Product type added successfully")
      setIsAddModalOpen(false)
    } catch (error) {
      notify("error", "Failed to add product type")
      console.error(error)
    } finally {
    ...
    return (
      <section className="h-full w-full">
        ...
        <div className="flex h-screen items-center justify-center">
          <div className="text-red-500">Failed to load product types</div>
        </div>
      </section>
    )
  }
      ...
      <div className="flex min-h-screen w-full flex-col">
        ...
        <div className="flex flex-col">
          <div className="flex items-center justify-between border-b px-16 py-4 max-sm:px-3">
            <p className="text-2xl font-medium">Product Types</p>
            ...
            <div className="w-full">
              <div className="mb-5 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm max-sm:text-xs">
                  <p className="text-grey-300">Product Types:</p>
                  {isLoading ? (
                    <div className="h-4 w-8 animate-pulse rounded bg-gray-200"></div>
                  ) : (
                    <p>{productTypes.length}</p>
                  )}
                </div>
              </div>
                      ...
                      <div key={type.id} className="rounded-lg bg-white p-4 shadow-lg">
                        <div className="flex gap-4">
                          <div className="bg-primary/10 relative flex h-12 w-12 items-center justify-center rounded-full">
                            <span className="text-primary text-lg font-semibold">{type.productTypeCode}</span>
                          </div>
                          <div className="flex-1">
                            <h3 className="text-base font-bold">{type.productTypeName}</h3>
                            <p className="text-sm text-gray-500">ID: {type.productTypeID}</p>
                            <p className="text-sm text-gray-500">Code: {type.productTypeCode}</p>
                            <p className="text-sm text-gray-500">Account Type: {type.accountType}</p>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {/* Pagination */}
              {!isLoading && productTypes.length > itemsPerPage && (
                <div className="flex items-center justify-between border-t px-4 py-3">
                  <div className="text-sm text-gray-700">
                    Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, productTypes.length)} of{" "}
                    {productTypes.length} entries
                  </div>
