...
const ActionDropdown: React.FC<ActionDropdownProps> = ({ account, onViewDetails }) => {
  ...
  const handleViewDetails = (e: React.MouseEvent) => {
    e.preventDefault()
    localStorage.setItem("selectedVirtualAccount", JSON.stringify(account))
        ...
        <div className="absolute right-0 z-[1000] mt-2 w-48 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            ...
            <button
              className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100"
              onClick={() => {
                console.log("Freeze account:", account.virtualAccountID)
                setIsOpen(false)
              }}
            >
              Freeze Account
            ...
            <button
              className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100"
              onClick={() => {
                console.log("Delete account:", account.virtualAccountID)
                setIsOpen(false)
              }}
            >
              Delete
...
const VirtualAccountsTable: React.FC = () => {
  ...
  const virtualAccounts = virtualAccountsResponse?.data || []
  const totalRecords = virtualAccountsResponse?.totalRecords || 0

  const getStatusStyle = (allowOverPay: boolean, allowUnderPay: boolean) => {
    return allowOverPay && allowUnderPay
      ? { backgroundColor: "#EEF5F0", color: "#589E67" }
      : { backgroundColor: "#F7EDED", color: "#AF4B4B" }
  }

  const toggleSort = (column: keyof VirtualAccount) => {
    ...
    return (
      <div className="flex h-60 flex-col items-center justify-center gap-2 bg-[#f9f9f9]">
        <div className="text-center">
          <EmptyState />
          <p className="text-xl font-bold text-[#D82E2E]">Failed to load virtual accounts.</p>
  ...
  return (
    <div className="flex-3 mt-5 flex flex-col rounded-md border bg-white p-5">
      {/* Header */}
      <div className="items-center justify-between border-b py-2 md:flex md:py-4">
        <p className="text-lg font-medium max-sm:pb-3 md:text-2xl">Virtual Accounts</p>
      ...
      {filteredAccounts.length === 0 ? (
        <div className="flex h-60 flex-col items-center justify-center gap-2 bg-[#f9f9f9]">
          <EmptyState />
          <p className="text-base font-bold text-[#202B3C]">No virtual accounts found.</p>
        </div>
      ) : (
        <>
          <div className="w-full overflow-x-auto border-l border-r bg-[#f9f9f9]">
              ...
              <thead>
                <tr>
                  <th
                    ...
                    onClick={() => toggleSort("virtualAccountID")}
                  >
              ...
              <tbody>
                {filteredAccounts.map((account, index) => (
                  <tr key={index}>
                    ...
                    <td className="whitespace-nowrap border-b px-4 py-2 text-sm">{account.email || "N/A"}</td>
                    <td className="whitespace-nowrap border-b px-4 py-2 text-sm">{account.phoneNumber || "N/A"}</td>
                    <td className="whitespace-nowrap border-b px-4 py-2 text-sm">{account.amount.toLocaleString()}</td>
                    <td className="whitespace-nowrap border-b px-4 py-2 text-sm">{account.accountNumber}</td>
                    <td className="whitespace-nowrap border-b px-4 py-2 text-sm">
                      <div
                        style={getStatusStyle(account.allowOverPay, account.allowUnderPay)}
                        className="flex items-center justify-center gap-1 rounded-full px-2 py-1"
                      >
                        <span
                          className="size-2 rounded-full"
                          style={{
                            backgroundColor: account.allowOverPay && account.allowUnderPay ? "#589E67" : "#AF4B4B",
                          }}
                        ></span>
                        {account.business}
                      </div>
